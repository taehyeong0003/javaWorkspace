...
public class C_Scanner {
	...
	public void inputTest1() {
		...
		// 안[스페이스]태형[엔터]
		String firstname = sc.next();		// 안
		String lastname = sc.nextLine();	// [스페이스]태형
		// nextLine함수는 호출 후 "버퍼"의 값을 비워준다 => [엔터]갑 소멸
		
		System.out.print("당신의 나이는 몇 살 입니까 : ");
		int age = sc.nextInt();	// 입력버퍼에 있는 값을 Int자료형으로 뽑아쓰겠다
		
		sc.nextLine();// 버퍼에 남은 엔터값 비워주기
	 ...
	 public void inputTest2() {
		 ...
		 */
		 ...
		 /*
		  * 주의사항
		  * 1) sc.nextXXX() : 메소드들 뒤에 sc.nextLine()메서드가 와야할 경우
		  * 				sc.nextLine()을 한번 더 호출하여 버퍼를 비워줘야 한다
		  * 2) 스캐너를 통해 '문자'값을 입력 받아야 될 경우
		  * 	sc.nextLine을 통해 문자열을 입력 받고, 그 후 char(인덱스)를 호출한다.
		  */
	 }
